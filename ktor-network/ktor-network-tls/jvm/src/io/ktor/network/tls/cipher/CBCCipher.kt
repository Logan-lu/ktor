...
internal class CBCCipher(
    ...
    override fun decrypt(record: TLSRecord): TLSRecord {
        ...
        val paddingLength = content[content.size - 1]
    ...
    private fun BytePacketBuilder.writePadding() {
        val paddingSize: Byte = (sendCipher.blockSize - (size + 1) % sendCipher.blockSize).toByte()
    ...
    private fun validatePadding(content: ByteArray, paddingStart: Int) {
        val padding = content[content.size - 1]
        for (i in paddingStart until content.size) {
            if (padding != content[i]) throw TLSException("Padding invalid: expected $padding, actual ${content[i]}")
        }
    }
    ...
    private fun validateMac(record: TLSRecord, content: ByteArray, macOffset: Int) {
        ...
        if (!expectedMac.contentEquals(actual)) throw TLSException("Failed to verify MAC content")
    }
}
